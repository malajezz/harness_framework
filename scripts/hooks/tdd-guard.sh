  ...
  cat << 'EOF'
{
  "hookSpecificOutput": {
    ...
    "permissionDecisionReason": "TDD GUARD: jq가 없어 가드를 실행할 수 없습니다. 'brew install jq' 후 다시 시도하세요."
  }
}
EOF
  exit 0
...
# 맨 `*test*`를 쓰지 않는다 — "latest"에 "test"가 들어 있어 lib/getLatestAnalysis.ts 같은
# 구현 파일이 테스트로 오인돼 가드를 통째로 우회한다. 확장자·디렉토리로만 판별한다.
case "$FILE_PATH" in
...
case "$FILE_PATH" in
  *.ts|*.tsx|*.js|*.jsx)
    ...
    # src/__tests__/ 루트 테스트 폴더 — 경로 기반으로 매칭한다.
    # BASENAME만 쓰면 app/api/*/route.ts 6개가 route.test.ts 하나로 전부 통과한다.
    if [ "$TEST_FOUND" = false ]; then
      PROJECT_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || echo ".")
      REL_PATH="${FILE_PATH#"${PROJECT_ROOT}/"}"
      REL_NO_EXT="${REL_PATH%.*}"
      REL_NO_SRC="${REL_NO_EXT#src/}"
      for EXT in ts tsx js jsx; do
        if [ -f "${PROJECT_ROOT}/src/__tests__/${REL_NO_SRC}.test.${EXT}" ]; then
          TEST_FOUND=true
          break
        fi
      done
